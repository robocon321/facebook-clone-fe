...
const LeftSidebar: React.FC = () => {
  const { receiveRequestFriendship, setReceiveRequestFriendship } = useContext(
    RelationshipContext
  ...
  return (
    <div
      className="p-2 w-full h-auto overflow-y-auto bg-white"
      style={{
    ...
    >
      ...
      <h3 className="font-bold">
        {receiveRequestFriendship.data.length} requests to make friends
      </h3>
      <Link className="mb-4 text-blue-600 text-sm hover:underline" href="#">
        Show your request you sent
      </Link>
      <ul>
        {receiveRequestFriendship.data.map((item) => (
          <ConfirmFriendCardHorizal
            key={item.accountId}
            onConfirm={onConfirm}
            onReject={onReject}
            account={item}
