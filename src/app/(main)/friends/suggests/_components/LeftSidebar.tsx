...
const LeftSidebar: React.FC = () => {
    const { recommendRequestFriendship, setRecommendRequestFriendship } = useContext(RelationshipContext) as RelationshipContextType;
    ...
    return (
        <div className="p-2 w-full bg-white">
            ...
            <ul>
                {
                    recommendRequestFriendship.data.map(item => <AddFriendCardHorizal key={item.accountId} onAddFriend={onAddFriend} onDelete={onDelete} account={item} />)
                }
            </ul>
        </div>
    );
