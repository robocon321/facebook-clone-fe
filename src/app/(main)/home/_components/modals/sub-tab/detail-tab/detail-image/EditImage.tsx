...
import { ImageModalType, ModalContextType } from "app/(main)/home/_type/ModalType";
        ...
        <div className={"relative p-8 bg-gray-800 w-full h-full flex justify-center items-center"}>
            {
                controlModalState.navTabDetailIndex == TAB_IMAGE_NAV_CODE.TAG ?
                    ...
                    controlModalState.navTabDetailIndex == TAB_IMAGE_NAV_CODE.TEXT ?
                        <div className="absolute bottom-[10px] left-1/2 transform -translate-x-1/2 text-white">Click any position on image to text your friends</div>
                        :
                        <></>
            }
            <div className="relative">
                <img
                    ref={imageRef}
                    onClick={(e) =>                         
                        editImageRef.current?.handleClick(e)
                    }
                    className="max-w-full cursor-pointer"
                    src={blobUrl} alt="Not found" />
                {
                    controlModalState.navTabDetailIndex == TAB_IMAGE_NAV_CODE.TAG ?
                        <TagControl fileModal={fileModal} ref={editImageRef} imageRef={imageRef} />
                        :
                        controlModalState.navTabDetailIndex == TAB_IMAGE_NAV_CODE.TEXT ?
                            <InsertTextControl fileModal={fileModal} ref={editImageRef} imageRef={imageRef} />
                            :
                            <></>
                }
            </div>
