...
const NavImageComponent: React.FC<NavPropsType> = (props) => {
  const { changeFieldDataFileModal, setControlModalState, controlModalState } =
  ...
  return (
    <div className="p-4">
      <textarea
        ...
        className="w-full p-2 outline-none resize-none border border-gray-300 hover:border-blue-300"
        rows={3}
      ...
      <ul className="mt-2">
        <li
          ...
          className={
            "flex items-center p-2 rounded hover:bg-gray-200 cursor-pointer mb-2" +
            (navTabDetailIndex == TAB_IMAGE_NAV_CODE.CROP ? " bg-blue-100" : "")
          }
        >
          <span className="mr-4">
            <RoundedIcon icon={faCropSimple} />
          </span>
          <span>Crop</span>
        </li>
        <li
          ...
          className={
            "flex items-center p-2 rounded hover:bg-gray-200 cursor-pointer my-2" +
            (navTabDetailIndex == TAB_IMAGE_NAV_CODE.ROTATE
              ? " bg-blue-100"
              : "")
          }
          ...
          <li className="flex flex-wrap border border-gray-200 p-2 max-h-[300px] text-sm">
            ...
            {fileModal.tags
              ...
              .map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center m-2 bg-blue-100 text-blue-600 min-w-[150px] rounded p-2"
                >
                  <div>
                    {item.account.firstName + " " + item.account.lastName}
                  </div>
                  <div
                    className="hover:bg-gray-200 rounded-full w-[25px] h-[25px] flex justify-center items-center cursor-pointer"
            ...
            <ul className="flex flex-wrap justify-between items-center">
              {colors.map((item) => {
                return (
                  <li
                    key={item}
                    onClick={() => changeColorSelectedText(item)}
                    className="w-[30px] h-[30px] rounded-full m-1"
        ...
        <li
          ...
          className={
            "flex items-center p-2 rounded hover:bg-gray-200 cursor-pointer mt-2" +
            (navTabDetailIndex == TAB_IMAGE_NAV_CODE.REPLACE
              ? " bg-blue-100"
              : "")
          }
