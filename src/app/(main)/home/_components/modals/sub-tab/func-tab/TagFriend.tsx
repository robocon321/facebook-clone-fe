...
export const ChildrenIconComponent: React.FC = () => {
  const { dataModalState, setDataModalState } = useContext(
    ModalContext
  ) as ModalContextType;
  ...
  const onAddFriendTag = (account: AccountResponse) => {
    ...
    setSearchFriends(
      searchFriends?.filter((item) => item.accountId != account.accountId)
    );
  };
  ...
  return (
    <div className="p-1">
      ...
      {isTagNonEmpty() && (
        <div className="border border-gray-200 my-2 max-h-[300px] text-sm p-2">
