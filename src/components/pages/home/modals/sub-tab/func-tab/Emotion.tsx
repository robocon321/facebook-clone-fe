...
import { EmotionType, ModalContextType } from 'types/pages/home/ModalType';
    ...
    return (
        <div className="p-1 max-h-[400px] overflow-auto">
            <div className="flex items-center p-2 border border-gray-200 bg-white rounded">
                ...
                <input onChange={onTextInputChange} type="text" className="outline-none grow" placeholder="Emotion's name" />
            </div>
            <ul className="grid grid-cols-2 gap-4 my-2">
                {
                    emotions?.filter(item => item.text.startsWith(searchText)).map(item => (
                        <li onClick={() => chooseEmotion(item)} key={item.emotion_id} className={"flex items-center p-2 hover:bg-gray-200 rounded cursor-pointer" + (item.emotion_id == dataModalState.emotion?.emotion_id ? " bg-gray-200" : "")}>
                            <span className="mr-2 w-[50px] h-[50px]"><img className="w-full h-full rounded-full" src={item.imageUrl} alt="Not found" /></span>
